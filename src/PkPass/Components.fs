...
type FloatingActionButton() =
    ...
    member val ButtonReference = HtmlRef() with get
...
type AddPassFloatingActionButton() =
    ...
    [<Literal>]
    let pkPassMimeType = "application/vnd.apple.pkpass"
                
    [<Literal>]
    let pkPassesMimeType = "application/vnd.apple.pkpasses"
    ...
    override this.View _ dispatch =
        concat {
            comp<FloatingActionButton> {
                on.task.click (fun _ ->
                    task {
                        do! this.JsRuntime.InvokeVoidAsync("getAndCacheFilesFromUser", fallBackInputReference.Value)
                        dispatch ()
                    })
            }
