namespace PkPass.Components

open System.Threading.Tasks
open Bolero
open Bolero.Html
open Microsoft.AspNetCore.Components
open Microsoft.JSInterop

module Elements =
    let private headerFields (logoText: string) =
        header {
            attr.``class`` "bg-gray-200 h-12 px-2 pt-2 pb-1 w-full flex justify-between items-center"

            div {
                attr.``class`` "bg-gray-300 w-full h-7 align-middle"

                p {
                    attr.``class`` "align-middle"
                    "Logo"
                }
            }

            div {
                attr.``class`` "bg-gray-300 w-full text-center h-7 align-middle"

                p {
                    attr.``class`` "align-middle"
                    logoText
                }
            }

            div {
                attr.``class`` "bg-gray-300 w-full text-end h-7 align-middle"

                p {
                    attr.``class`` "align-middle"
                    "Header fields"
                }
            }
        }

    let private barcode () =
        article {
            attr.``class`` "bg-gray-200 px-2 pb-2 pt-1"

            div {
                attr.``class`` "bg-gray-300 rounded p-3"
                div { attr.``class`` "aspect-square w-60 bg-gray-400 m-auto rounded-xl" }
            }
        }


    let boardingPass () =
        article {
            attr.``class``
                "origin-top transition-transform scale-[var(--scale)] bg-gray-100 rounded-lg aspect-[1/1.62] overflow-hidden flex flex-col justify-between mb-10 shadow-xl border-2 border-blue-300"

            section {
                // Header fields
                headerFields "Boarding pass"
                // Primary fields
                article {
                    attr.``class`` "bg-gray-200 rounded flex justify-between h-20 px-2 py-1"
                    div { attr.``class`` "aspect-video bg-gray-300 rounded" }
                    div { attr.``class`` "aspect-video bg-gray-300 rounded" }
                }

                // Auxiliary fields
                article {
                    attr.``class`` "bg-gray-200 h-10 px-2 py-1"
                    div { attr.``class`` "bg-gray-300 w-full h-full rounded" }
                }

                article {
                    attr.``class`` "bg-gray-200 h-10 px-2 py-1"
                    div { attr.``class`` "bg-gray-300 w-full h-full rounded" }
                }
            }

            section {
                article {
                    attr.``class`` "bg-gray-200 h-8 px-2 py-1"
                    div { attr.``class`` "bg-gray-300 w-full h-full rounded" }
                }

                barcode ()
            }
        }

    let coupon () =
        article {
            attr.``class``
                "origin-top transition-transform scale-[var(--scale)] bg-gray-100 rounded-lg aspect-[1/1.62] overflow-hidden flex flex-col justify-between mb-10 shadow-xl border-2 border-blue-300"

            section {
                headerFields "Coupon"

                // Primary fields
                article {
                    attr.``class`` "bg-gray-200 rounded h-32 py-1"

                    div {
                        // Strip image
                        attr.``class`` "bg-gray-300 rounded-none p-2 w-full h-full"
                        div { attr.``class`` "w-full h-full bg-gray-400 rounded" }
                    }
                }

                // Secondary and auxiliary fields
                article {
                    attr.``class`` "bg-gray-200 h-12 px-2 py-2"
                    div { attr.``class`` "bg-gray-300 w-full h-full rounded" }
                }
            }
            
            section {
                barcode ()
            }
        }

    let eventTicketWithBackgroundImage () =
        article {
            attr.``class``
                "origin-top transition-transform scale-[var(--scale)] bg-gray-100 rounded-lg aspect-[1/1.62] overflow-hidden flex flex-col justify-between mb-10 shadow-xl border-2 border-blue-300"

            section {
                headerFields "Event ticket 1"

                article {
                    attr.``class`` "bg-gray-200 rounded h-32 py-1"

                    div {
                        attr.``class`` "flex gap-1 bg-gray-300 h-full w-full p-1"

                        div {
                            attr.``class`` "flex flex-col gap-1 bg-gray-400 h-full w-full p-1"
                            div { attr.``class`` "bg-gray-500 h-2/3 w-full p-1" }
                            div { attr.``class`` "bg-gray-500 h-1/3 w-full p-1" }
                        }

                        div { attr.``class`` "h-full aspect-[3/4] bg-gray-400 rounded-lg" }
                    }
                }

                article {
                    attr.``class`` "bg-gray-200 h-12 px-2 py-2"
                    div { attr.``class`` "bg-gray-300 w-full h-full rounded" }
                }
            }

            section {
                barcode ()
            }
        }

    let eventTicketWithStripImage () =
        article {
            attr.``class``
                "origin-top transition-transform scale-[var(--scale)] bg-gray-100 rounded-lg aspect-[1/1.62] overflow-hidden flex flex-col justify-between mb-10 shadow-xl border-2 border-blue-300"

            section {
                headerFields "Event ticket 2"

                // Primary fields
                article {
                    attr.``class`` "bg-gray-200 rounded h-32 py-1"

                    div {
                        // Strip image
                        attr.``class`` "bg-gray-300 rounded-none p-2 w-full h-full"
                        div { attr.``class`` "w-full h-full bg-gray-400 rounded" }
                    }
                }

                article {
                    attr.``class`` "bg-gray-200 h-12 px-2 py-2"
                    div { attr.``class`` "bg-gray-300 w-full h-full rounded" }
                }

                article {
                    attr.``class`` "bg-gray-200 h-12 px-2 py-2"
                    div { attr.``class`` "bg-gray-300 w-full h-full rounded" }
                }
            }

            section {
                barcode ()
            }
        }
        
    let genericPass () = 
        article {
            attr.``class``
                "origin-top transition-transform scale-[var(--scale)] bg-gray-100 rounded-lg aspect-[1/1.62] overflow-hidden flex flex-col justify-between mb-10 shadow-xl border-2 border-blue-300"

            section {
                headerFields "Generic pass"

                article {
                    attr.``class`` "bg-gray-200 rounded h-32 py-1"
                    div {
                        attr.``class`` "flex gap-1 bg-gray-300 h-full w-full p-1"
                        div { attr.``class`` "gap-1 bg-gray-400 h-full w-full p-1 rounded-lg" }
                        div { attr.``class`` "h-full aspect-[3/4] bg-gray-400 rounded-lg" }
                    }
                }

                article {
                    attr.``class`` "bg-gray-200 h-12 px-2 py-2"
                    div { attr.``class`` "bg-gray-300 w-full h-full rounded" }
                }
            }

            section {
                barcode ()
            }
        }
